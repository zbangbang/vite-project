...
export default class MapBook {
  ...
  private loadTile() {
    const { tx, ty } = Projection.LatlonToTileXY(...this.options.center, this.options.zoom)
    let centerTilePos = [tx * 256, ty * 256]
    let centerPos = Projection.LatlonToPixelXY(...this.options.center, this.options.zoom)
    let offset = [
    ...
    for (let i = -minx; i <= maxx; i++) {
      for (let j = -miny; j <= maxy; j++) {
        ...
        cacheKey = `${row}_${col}_${this.options.zoom}`
  ...
  private initEvent() {
    ...
    this.canvas.onmousemove = e => {
      ...
      let resolution = Projection.getResolution(this.options.zoom)
    ...
    this.canvas.onwheel = (e: WheelEvent) => {
      ...
      if (this.lastZoom === this.options.zoom) return
      this.lastZoom = this.options.zoom
