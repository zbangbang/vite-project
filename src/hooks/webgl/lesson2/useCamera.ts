...
import { ICameraParam, IThreeParam } from "./types"
 ...
 * @return {*}
 */
export default function useCameraHook(gl: WebGLRenderingContext, program: WebGLProgram) {
  // 绘制三维
  const drawThree = (angle: number) => {
    ...
    for (let ii = 0; ii < posArr.length; ii += 3) {
      let vector = m4.transformVector([posArr[ii + 0], posArr[ii + 1], posArr[ii + 2], 1] as any, matrix);
      posArr[ii + 0] = vector[0];
      posArr[ii + 1] = vector[1];
      posArr[ii + 2] = vector[2];
    }
    ...
    // 相机位置变换
    let cm = m4.yRotation((angle / 180) * Math.PI)
    cm = m4.translate(cm, 0, 0, 200 * 1.5)

    // 逆矩阵
    let vm = m4.inverse(cm)

    // 物体变换矩阵
    let pvm = m4.multiply(pm, vm)
    ...
    for (let i = 0; i < 5; i++) {
      ...
      let x = Math.cos(ang) * 200
      let z = Math.sin(ang) * 200

      let m = m4.translate(pvm, x, 0, z)
      gl!.uniformMatrix4fv(u_Matrix, false, m as any)
