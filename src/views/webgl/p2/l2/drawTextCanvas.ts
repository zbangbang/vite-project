/*
 * @FilePath: drawTextCanvas.ts
 * @Author: @zhangl
 * @Date: 2025-08-11 14:46:58
 * @LastEditTime: 2026-01-23 11:23:17
 * @LastEditors: @zhangl
 * @Description: 多几何体
 */
...
export const initDrawTextCanvasFrameBuffer = (gl: WebGL2RenderingContext) => {
  ...
  const render = (time?: any) => {
    ...
    modelList.forEach((model) => {
      ...
      const pos = twgl.m4.transformPoint(
        uniforms.u_mvpMatrix,
        twgl.v3.create(8, 8, 0)
      )
      pos[0] /= pos[2]
      pos[1] /= pos[2]

      let x = (pos[0] * 0.5 + 0.5) * gl.canvas.width
      let y = (pos[1] * -0.5 + 0.5) * gl.canvas.height
...
export const clearDrawTextCanvasFrameBuffer = () => {
